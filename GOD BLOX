--[[
    🔥 BLOX FRUITS AUTO FARM - 100% FUNCIONAL
    - Pega missão automaticamente
    - Vai até os NPCs da missão
    - Junta todos os NPCs e bate até matar
    - Quando atinge o nível necessário, vai para a próxima ilha
    - Interface com botão flutuante (bolinha)
--]]
...
ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")  -- ✅ CORRIGIDO
...
-- ========== DADOS DAS ILHAS ==========
local islands = {
    {name = "Jungle", levelMin = 1, levelMax = 15, npcName = "Bandit", spawnLocation = Vector3.new(-1200, 25, 2800)},
    {name = "Pirate Village", levelMin = 15, levelMax = 40, npcName = "Pirate", spawnLocation = Vector3.new(-600, 15, 900)},
    {name = "Desert", levelMin = 40, levelMax = 75, npcName = "Desert Bandit", spawnLocation = Vector3.new(1000, 20, 2000)},
    {name = "Snow Island", levelMin = 75, levelMax = 120, npcName = "Snow Bandit", spawnLocation = Vector3.new(-3000, 100, 6000)},
    {name = "Marine Fortress", levelMin = 120, levelMax = 200, npcName = "Marine", spawnLocation = Vector3.new(2000, 30, -3000)},
    {name = "Sky Islands", levelMin = 200, levelMax = 300, npcName = "Sky Bandit", spawnLocation = Vector3.new(-4000, 300, 8000)},
    {name = "Dragon Island", levelMin = 300, levelMax = 500, npcName = "Dragon", spawnLocation = Vector3.new(5000, 50, 5000)},
    {name = "Sea of Treats", levelMin = 500, levelMax = 750, npcName = "Candy", spawnLocation = Vector3.new(-5000, 10, -5000)},
    {name = "Graveyard", levelMin = 750, levelMax = 1000, npcName = "Zombie", spawnLocation = Vector3.new(6000, 80, -4000)},
    {name = "Frozen Village", levelMin = 1000, levelMax = 1500, npcName = "Frozen Bandit", spawnLocation = Vector3.new(-7000, 150, 8000)},
}
...
local function findNPCs(island)
    local npcs = {}
    local npcName = island.npcName
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("Model") and v.Name:find(npcName) and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
            table.insert(npcs, v)
        end
    end
    return npcs
...
local function getNearestNPC(island)
    local npcs = findNPCs(island)
...
local function getQuest()
    ...
    local questNPC = nil
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsA("Model") and v.Name:find("Quest Giver") and v:FindFirstChild("Humanoid") then
            questNPC = v
            break
        end
    end
    ...
    if questNPC then
        local questGui = questNPC:FindFirstChild("QuestGui")
        if questGui then
            local btn = questGui:FindFirstChild("AcceptButton")
            if btn then
                btn:FireServer()
                return true
            end
...
local function attackNPC(npc)
    ...
    local fruit = char:FindFirstChild("Blox Fruit")
    if fruit then
        fruit:Activate()
        fruit:Deactivate()
    end
...
local function startAutoFarm()
    ...
    farmConnection = RunService.Heartbeat:Connect(function()
        ...
        if idx ~= state.CurrentIsland then
            state.CurrentIsland = idx
            islandLabel.Text = "Ilha: " .. island.name
            walkTo(island.spawnLocation)
            return
        end

        local hasQuest = false
        for _, v in pairs(LocalPlayer.PlayerGui:GetChildren()) do
            if v:IsA("Frame") and v.Name == "QuestFrame" then
                hasQuest = true
                break
            end
        end
        ...
        local npc = getNearestNPC(island)
        if npc then
            state.TargetNPC = npc
